...
namespace Everhour.Net
{
    public partial class EverhourClient
    {
        ...
        /// <param name="projectId">Project ID</param>
        /// <param name="type">Budget Type</param>
        public async Task<SetProjectBillingResponse> SetProjectBillingByFlatRateAsync(string projectId, int rate)
        {
            if (string.IsNullOrEmpty(projectId)) throw new ArgumentException("Value cannot be null or empty.", nameof(projectId));

            return await ExecuteAsync<SetProjectBillingResponse>(UpdateRequest<SetProjectBillingRequest>(PROJECTS_PATH, projectId, "billing", new SetProjectBillingRequest() { Id = projectId, Type = BillingType.FLAT_RATE, Rate = rate })).ConfigureAwait(false);
        }
        ...
        /// <param name="projectId">Project ID</param>
        /// <param name="type">Budget Type</param>
        public async Task<SetProjectBillingResponse> SetProjectBillingByUserRateAsync(string projectId, IList<UserWithRate> users)
        {
            if (string.IsNullOrEmpty(projectId)) throw new ArgumentException("Value cannot be null or empty.", nameof(projectId));
            if (users == null) throw new ArgumentNullException("Value cannot be null.", nameof(users));
            if (users.Count == 0) throw new ArgumentException("Value cannot be empty.", nameof(users));

            return await ExecuteAsync<SetProjectBillingResponse>(UpdateRequest<SetProjectBillingRequest>(PROJECTS_PATH, projectId, "billing", new SetProjectBillingRequest() { Id = projectId, Type = BillingType.USER_RATE, Users = users })).ConfigureAwait(false);
        }
